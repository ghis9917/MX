...
- `Bonus features (?)`:
    - `Confidence Score`: This is a score generated by the agent when providing the response which is defined in the structured output of the mode. Unfortunately as the whole agent runs async and the logprobs are not yet available through pydantic ai run and run_stream methods, I decided to go with this less technical way of determining the model confidence.